...
// DEPRECATED: This is being replaced by ModelNew which uses services
type Model struct {
	...
	// For migration to new architecture
	eventChan  <-chan interface{}
	repoStore  logic.RepositoryStore
	groupStore logic.GroupStore
}

// NewLegacyModel creates a new UI model using the legacy architecture
// DEPRECATED: Use the new service-based model instead
func NewLegacyModel(
	cfg *config.Config,
	...
	m := NewModel(eventBus, cfg)
	
	// Store the event channel for future use
	m.eventChan = eventChan
	
	// Create adapters to bridge legacy state to new stores
	adapter := adapters.NewStateToStoreAdapter(m.state)
	
	// Wire up the stores (for gradual migration)
	m.repoStore = adapter.AsRepositoryStore()
	m.groupStore = adapter.AsGroupStore()
	
	return m
}

// NewModel creates a new UI model (legacy implementation)
