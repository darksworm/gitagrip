...
impl App {
    ...
    /// Get the total number of display items (repos + group headers) in organize mode
    pub fn total_display_items(&mut self) -> usize {
        if self.repositories.is_empty() {
            return 1; // "Scanning..." or "No repos" message
        }
        
        // Use the same grouping logic as UI rendering
        let mut all_groups = Vec::new();
        ...
        // Count group headers + repositories
        let mut total_items = 0;
        for (_, repos) in all_groups {
            total_items += 1; // Group header
            total_items += repos.len(); // Repository items
        }
    ...
    pub fn ensure_selection_visible(&mut self) {
        ...
        // Use the correct total display items count
        let total_content_lines = self.total_display_items();
        
        // Only scroll if we have more content than can fit
        if total_content_lines > estimated_visible_height {
            // In organize mode, current_selection directly corresponds to display line
            let selection_line = self.current_selection;
    ...
    pub fn handle_organize_key(&mut self, key: crossterm::event::KeyCode) -> Result<bool> {
        ...
        match key {
            crossterm::event::KeyCode::Down | crossterm::event::KeyCode::Char('j') => {
                ...
                let total_items = self.total_display_items();
                if self.current_selection + 1 < total_items {
